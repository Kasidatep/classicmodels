...
<html>
<head>
    <title>ProductList - ClassicModel</title>
        ...
        <c:forEach begin="1" end="${totalPage}" varStatus="vs">
            <c:choose>
                <c:when test="${vs.count==page}">
                    <div class="btn btn-dark"> ${vs.count} </div>
                </c:when>
                <c:otherwise>
                    <a href="ProductList?page=${vs.count}&size=${pageSize}">
                        <div class="btn btn-outline-dark color-dark">
                                ${vs.count}
                        </div>
                    </a>
                </c:otherwise>
